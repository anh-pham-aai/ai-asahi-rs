...
@app.post("/rag/query")
async def rag_query(body: QueryRequestBody):
    """
    Query the RAG system with a question, using documents from a specific file_id.
    """
    try:
        ...
        logger.error(
            "Error in RAG query | File ID: %s | Query: %s | Error: %s | Traceback: %s",
            body.file_id,
            body.query,
            str(e),
            traceback.format_exc(),
...
@app.post("/rag/query_multiple")
async def rag_query_multiple(body: QueryMultipleBody):
    """
    Query the RAG system with a question, using documents from multiple file_ids.
    """
    try:
        result = await query_rag(
            question=body.query,
        ...
        logger.error(
            "Error in RAG query multiple | File IDs: %s | Query: %s | Error: %s | Traceback: %s",
            body.file_ids,
            body.query,
            str(e),
            traceback.format_exc(),
...
@app.post("/rag/query_all")
async def rag_query_all(body: QueryAllBody):
    """
    Query the RAG system with a question, using all available documents.
    """
    try:
        result = await query_rag(
            question=body.query,
        ...
        logger.error(
            "Error in RAG query all | Query: %s | Error: %s | Traceback: %s",
            body.query,
            str(e),
            traceback.format_exc(),
        )
